...
public class TrampolineCompiler {
	...
	public void compile(ModuleBuilder mb, Clazz currentClass, Trampoline t) {
	...
	private void createTrampolineAliasForMethod(Invoke t, SootMethod rm) {
		String fnName = null;
		SootMethod divrtMethod = t.getResolvedMethod();

		if(divrtMethod != null) {
			if(divrtMethod.isSynchronized())
				fnName = Symbols.synchronizedWrapperSymbol(divrtMethod);
			else
				fnName = Symbols.methodSymbol(divrtMethod);
		}
		else {
			if (t instanceof Invokeinterface) {
				fnName = Symbols.lookupWrapperSymbol(rm);
					...
					&& (rm.isAbstract() || hasSubClasses(rm.getDeclaringClass()))) {
					fnName = Symbols.lookupWrapperSymbol(rm);
